/*
    This is a very simple demonstration of a while loops. Same as JavaScript/C
*/

contract BasicIterator {

    address creator;    // reserve one "address"-type spot
    uint8[10] integers; // reserves a chunk of storage for 10 8-bit unsigned integers in an array

    function BasicIterator() {
        creator = msg.sender;   // set the creator address
        uint8 x = 0;            // initialize an 8-bit, unsigned integer to zero
        while(x < integers.length) {
            // the variable integers was initialized to length 10
    ...
    function getSum() constant returns(uint) {
        // "constant" just means this function returns something to the caller 
